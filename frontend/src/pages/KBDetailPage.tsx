  ...
  return (
    <div className="h-screen flex flex-col bg-surface-950">
      ...
      <header className="h-14 border-b border-surface-800 flex items-center px-4 gap-4 shrink-0 bg-surface-950/80 backdrop-blur-xl">
        ...
        <h1 className="font-semibold text-surface-200 truncate flex-1">
          {currentKB?.name || t('common.loading')}
        </h1>
        <LangSwitcher testId="kb-detail-lang-switcher" />
        <button onClick={handleDeleteKB} className="btn-ghost p-2 text-red-400" title={t('common.delete')}>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-[35%] min-w-[320px] border-r border-surface-800 flex flex-col overflow-hidden">
          {/* Upload area */}
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {docLoading ? (
              ...
              <div className="space-y-2">
                {documents.map((doc) => (
                  <div key={doc.id} className="bg-surface-900 border border-surface-800 rounded-lg p-3 hover:border-surface-700 transition-colors">
        ...
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="flex-1 flex min-h-0">
            ...
            <div className="w-56 border-r border-surface-800 shrink-0 hidden sm:block">
              <SessionList
                sessions={sessions}
              ...
              <div className="flex-1 overflow-y-auto px-4 py-4">
                {!currentSession ? (
                  <div className="flex flex-col items-center justify-center h-full text-center">
                    ...
                    <p className="text-surface-500 text-sm">{t('chat.startHint')}</p>
                    <button
                      type="button"
                    ...
                    {displayMessages.map((msg) => (
                      <div key={msg.id} className={`flex gap-3 py-3 ${msg.role === 'user' ? '' : 'bg-surface-900/50 -mx-4 px-4 rounded-lg'}`}>
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 mt-0.5 ${
                          msg.role === 'user' ? 'bg-surface-700' : 'bg-brand-500/10'
                        }`}>
                          {msg.role === 'user'
            ...
            {showRefs && (
              <div className="w-64 border-l border-surface-800 shrink-0 hidden xl:block overflow-y-auto">
                <div className="flex items-center justify-between px-3 py-3 border-b border-surface-800">
                  <h3 className="text-xs font-semibold text-surface-400">{t('chat.references')}</h3>
